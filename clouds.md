...
The entire musical score is treated as a matrix $$P$$, 
generated by concatenating blocks:


$$ P = \bigsqcup_{p=1}^{H} t_p \cdot (M \odot S_p) $$

Donde:
- $$(H)$$ es el número de imágenes
- $$(t_p)$$ son los triggers temporales
- $$(M)$$ es la máscara musical
- $$(S_p)$$ son las características espectrales
- $$(\odot)$$ denota el producto de Hadamard
- $$(\bigsqcup)$$ es la concatenación vertical
...
$P = \bigsqcup_{p=1}^{H} t_p \cdot (M \odot S_p)$
Here, 
 elegantly encapsulate the process of triggering and arranging sonic events based on visual data. This formulation makes the algorithm agnostic to musical system, allowing it to adapt to different tuning scales or traditions.
